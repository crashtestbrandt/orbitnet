...
func join(address: String) -> bool:
	...
	var host: String = NetTransport.target_address(address)
	var port: int = NetTransport.target_port(address)
	...
	print("RTS: joining %s:%d (%s)" % [host, port, NetTransport.preferred_kind_name()])
	return true
