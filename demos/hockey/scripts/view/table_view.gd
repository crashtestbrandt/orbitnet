...
## PuckPhysics be plain 2D maths over an axis-aligned rectangle.
##
...
func _build_surface() -> void:
	...
	_slab("CentreLine", Vector3(width, 0.014, 0.006), Vector3(0.0, 0.001, 0.0), Color(0.30, 0.36, 0.46))
	_ring("CentreSpot", Color(0.30, 0.36, 0.46))
	...
	var segment_centre: float = HockeyConfig.GOAL_HALF_WIDTH + segment * 0.5
	for team: int in 2:
		var z: float = TableGeometry.goal_line_z(team) + HockeyConfig.end_sign(team) * RAIL_THICKNESS * 0.5
		for side: int in 2:
			var x: float = segment_centre if side == 0 else -segment_centre
			_slab("Rail%d_%d" % [team, side], Vector3(segment, RAIL_HEIGHT, RAIL_THICKNESS),
				Vector3(x, RAIL_HEIGHT * 0.5, z), Color(0.55, 0.60, 0.68))
		# The mouth itself, painted in the defending team's colour so which end is yours is readable at a
...
func _slab(slab_name: String, size: Vector3, at: Vector3, colour: Color) -> void:
	var mesh: BoxMesh = BoxMesh.new()
	mesh.size = size
	var instance: MeshInstance3D = MeshInstance3D.new()
	instance.name = slab_name
	instance.mesh = mesh
	instance.material_override = unshaded(colour)
	instance.position = at
	board.add_child(instance)

func _ring(ring_name: String, colour: Color) -> void:
...
## on a software CI runner and nothing here is trying to look lit -- a flat colour reads the table's geometry
