...
no longer be named without a rate. Both are parse-time breaks in a game that calls them. Nothing on `Net`
moved — the 1.0 freeze covers that surface, and this one is still open to change before then.

| Change | What breaks | What to do |
|---|---|---|
| **`NetLagComp.MAX_INTERP_TICKS` is removed** | a game that read the flat 8-tick ceiling | call `NetLagComp.max_interp_ticks(tick_hz)` for the ceiling at a given rate |
| **`NetLagComp.observed_interp_for_band()` takes a third argument**, `tick_hz` | the two-argument call no longer parses | pass the rate the loop is running at, `float(Net.effective_tickrate())` |
