...
/// | 3 | Every datagram but the handshake carries a sequence number and a MAC (`crate::auth`); the handshake carries the session key (the 16 bytes major 7 renames [`crate::codec::Handshake::joiner_nonce`]) and no longer carries a session-wide schema hash. |
...
pub const PROTOCOL_VERSION: u32 = 0x0009_0000;
