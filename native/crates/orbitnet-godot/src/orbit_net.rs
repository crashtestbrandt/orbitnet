...
impl OrbitNet {
    ...
    fn update_interest(
        ...
        for &peer_id in peer_ids {
            ...
            // Membership is a separate axis and is not switched off here — an observer with no body
            // reads as MEMBERSHIP_GLOBAL, which matches every world, so that case fails open too.
            let center = match observer {
                Some(observer) if cfg.enter_radius > 0.0 => observer.center,
...
mod tests {
    use super::{
        ...
        owned_rows_into, owned_rows_of, EntityRow, OrbitNet, PeerObserver, PeerState, RxOutcome,
